...
from spmg_tkinter.spmg_tkinter_main import bind_all_children, get_root_of_widget
...
class Rearrangeable(object):
    ...
    def __init__(self, parent:tk.Widget, frame_height:int=50, frame_padding:int=1, frame_bg_color="gray25", starting_frame_data:list=[]):
        self.parent = parent
        ...
        self.starting_frame_data = starting_frame_data
        ...
        # update so the canvas height is set
        self.canvas.update()
        ...
        for frame_data in self.starting_frame_data:
            self.add_frame(frame_data=frame_data)
    ...
    def add_frame(self, frame_data) -> tk.Frame:
        ...
        self.create_frame(frame, frame_data)
    ...
    def create_frame(self, frame:tk.Frame, frame_data) -> None:
        """creates the contents of the frame."""
        label = tk.Label(frame, text=F"{frame_data}: Frame {len(self.frames)+1}", font=('Consolas', 24), fg='white', background='gray25')
        label.pack(side='left')

    def frame_moved(self, start_index:int, end_index:int) -> None:
        """called after frame is moved."""
        print(F"started:{start_index}, ended:{end_index}")

    def sort_frames(self):
        """organizes `frames` position based of there index in `self.frames`."""
    ...
    def mouse_motion(self, event:tk.Event, frame:tk.Frame):
        if Rearrangeable.frame_dragging == frame:
            ...
            if abs(Rearrangeable.last_index-index) > 1:
                ...
                self.frame_entered_index(Rearrangeable.last_index, rounded_index)
                # update last index.
                Rearrangeable.last_index = rounded_index

    def frame_entered_index(self, old_index:int, new_index:int):
        """called when frame dragging enters new index."""
        total_frames = len(self.frames)
        # if dragging outside the `scrollable_frame` boundaries, return.
